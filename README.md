...
```
### ◾️今後の開発について
#### ログイン後トップページをダッシュボード化
現在の自己ベスト記録や、次の大会に向けた目標設定など、
競技力向上へのモチベーションを高めるためのダッシュボード機能を作成します。
#### グラフを増やす
スクワット、デッドリフト、ベンチプレス各種目の成績の推移が見れるようにグラフを作成します
#### テストコード
リファクタリングを安全に行うため、テストカバレッジを向上させるべく、テストコードを充実させます。
